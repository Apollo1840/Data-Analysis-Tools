...
df[x_attr] = list(pd.cut(df[cut_column], bins=[0, 1, 2, 4, 8, 50],
...
p = heatmap_2attr_p(df, 'actor_1_facebook_likes_level', 'facenumber_in_poster_level', func=np.mean, value='gross',
                    order_type=[2, 2])
p.render()

h2 = heatmap_2attr(df, 'actor_1_facebook_likes_level', 'facenumber_in_poster_level', func=np.mean, value='gross',
                   order_type=[3, 3])

heatmap_2attr(df, 'actor_1_facebook_likes_level', 'facenumber_in_poster_level', func=np.mean, value='gross',
              order_type=[3, 3])
p = heatmap_2attr_p(df, 'actor_1_facebook_likes_level', 'facenumber_in_poster', func=np.ma.median, value='gross',
                    order_type=[2, 1])
p.render()
...
for i in g_axis:
    ...
    bubble.add(i, dfv.values, is_visualmap=True,
               ...
               visual_range=[np.nanmin(dfv[attrs[2]]), np.nanmax(dfv[attrs[2]])], symbol_size=5,
...
#############################################################################
dfv = df.loc[:,
      ['imdb_score', 'profits', 'actor_1_facebook_likes', 'cast_total_facebook_likes', 'director_facebook_likes',
       'diversity', 'num_user_for_reviews']]
...
cluster_dic = {0: 'good_cast', 1: 'famous_actor_1', 2: 'unpopular', 3: 'extrodinary_cast', 4: 'famous_director',
               5: 'normal', np.nan: 'extra'}
...
dfv = df.groupby(x_attr)[
    'director_facebook_likes', 'actor_1_facebook_likes', 'actor_2_facebook_likes', 'actor_3_facebook_likes', 'cast_FL_except_a1'].agg(
    dp.mean)
...
print(len(nodes))
print(
    len(set([links_v[i]['source'] for i in range(len(links_v))] + [links_v[i]['target'] for i in range(len(links_v))])))
